...
<form id="payment-form">
    <div class="bg-white rounded-lg border bg-card text-card-foreground shadow-sm w-full max-w-md mx-auto my-20" data-id="1" data-v0-t="card">
        ...
        <div class="flex flex-col space-y-1.5 p-6" data-id="2">
            <h3 class="text-2xl font-semibold leading-none tracking-tight">Complete Your Purchase</h3>
            <p class="text-sm text-muted-foreground">Choose a product and enter your payment details.</p>
        </div>
        
        <div class="p-6 space-y-4">
            <!-- Product Selection -->
            <div class="space-y-2">
                <label for="product-selection" class="text-sm font-medium">Product</label>
                <select id="product-selection" name="product" class="w-full rounded-md border bg-background px-3 py-2 text-sm">
                    @foreach($products as $productId => $productName)
                        <option value="{{ $productId }}" data-price="{{ $productPrices[$productId] ?? 0 }}">{{ $productName }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <!-- Cardholder Name -->
            <div>
                <label for="card-holder-name" class="text-sm font-medium">Cardholder Name</label><br>
                <input id="card-holder-name" type="text" class="w-full rounded-md border bg-background px-3 py-2 text-sm">
            </div>
...
<script>
    document.addEventListener("DOMContentLoaded", function () {
        ...
        document.getElementById("subscription-product-selection").addEventListener("change", function () {
            const productId = this.value;
            if (productId !== "none") {
                // Make an AJAX call to get the PaymentIntent for the selected product
                fetch("{{ route('create.paymentIntent') }}", {
                    method: "POST",
                    headers: {
                ...
                .then(data => {
                    if (data.client_secret) {
                        // Confirm the PaymentIntent with the client_secret
